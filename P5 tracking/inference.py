...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            dist[ghostPosition] = prob * self.beliefs[ghostPosition]
        dist.normalize()
        self.beliefs = dist
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.particles:
            ...
            for i in range(self.numGhosts):
                prob *= self.getObservationProb(observation[i], pacmanPosition, ghostPosition[i], self.getJailPosition(i)) 
            dist[ghostPosition] += prob
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
