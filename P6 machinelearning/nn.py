...
class Graph(object):
    ...
    def step(self, step_size):
        ...
        for variable in self.variables:
            variable.data -= self.get_gradient(variable)*step_size
...
class SquareLoss(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        dimensions = inputs[0].shape
        n = dimensions[0]*dimensions[1]
        return [gradient*(inputs[0]-inputs[1])/(dimensions[0]*dimensions[1]), -gradient*(inputs[0]-inputs[1])/(dimensions[0]*dimensions[1])]
