        ...
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ghostHouse = \
                    ...
                    assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL and topRight or \
                    assignment[GHOST_HOUSE_VAR] == BOTTOM_LEFT_VAL and bottomLeft or \
                    assignment[GHOST_HOUSE_VAR] == BOTTOM_RIGHT_VAL and bottomRight
                foodHouse = \
                    assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and topLeft or \
                    assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and topRight or \
                    assignment[FOOD_HOUSE_VAR] == BOTTOM_LEFT_VAL and bottomLeft or \
                    assignment[FOOD_HOUSE_VAR] == BOTTOM_RIGHT_VAL and bottomRight
                if foodHouse:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment in factors.getAllPossibleAssignmentDicts():
        tempProb = factors.getProbability(assignment)
        if tempProb > prob:
            prob = tempProb
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        probFoodLeft, probFoodRight = 0, 0
        for assignment in factors.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                probFoodLeft = factors.getProbability(assignment)
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                probFoodRight = factors.getProbability(assignment)
        leftExpectedValue = probFoodLeft * WON_GAME_REWARD + (1  - probFoodLeft) * GHOST_COLLISION_REWARD
        rightExpectedValue = probFoodRight * WON_GAME_REWARD + (1 - probFoodRight) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            ...
            MaxEnterVal = max(self.computeEnterValues(evidence, enterEliminationOrder))
            expectedValue += MaxEnterVal * prob
