...
def joinFactors(factors):
    ...
    for factor in factors:
        ...
        tempConditioned = factor.conditionedVariables()
        unconditioned.extend(tempUnconditioned)
        for conditionedVar in tempConditioned:
            if not conditionedVar in conditioned:
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            prob = 0
            for varDomain in variableDomainsDict[eliminationVariable]:
                assignment[eliminationVariable] = varDomain
                prob += factor.getProbability(assignment)
            newFactor.setProbability(assignment, prob)
        return newFactor
    return eliminate
...
def normalize(factor):
    ...
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        prob = factor.getProbability(assignment)
        newFactor.setProbability(assignment, prob / probSum)
    return newFactor
