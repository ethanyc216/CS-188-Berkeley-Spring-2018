...
class CaptureAgent(Agent):
  ...
  def __init__( self, index, gameState, timeForComputing = .1 ):
    """
  ...
  def registerInitialState(self, gameState):
    ...
    self.distancer.getDistance(p1, p2, walls)
    """
  ...
  def choosePowers(self, gameState, powerLimit):
    ...
    It should return a dictionary of powers within the time limit.
    """
    util.raiseNotDefined()
  ...
  def getOpponentTeamPowers(self, gameState):
    """
    Returns the list of powers of your opponent agents
    """
    return [gameState.getOpponentAgentPowers(i) for i in self.getOpponents(gameState)]
  ...
  def getMazeDistance(self, pos1, pos2, state):
    """
    ...
    """
    d = self.distancer.getDistance(pos1, pos2, state.data.walls)
    return d
  ...
  #Note: These functions are slightly different than the released versions
  #Specifically, these write to a location defined in capture.py instead of "state"
  #This is because the cwd for matches is not the same as the bot directory
  def writeState(self, objectToWrite):
    if len(pickle.dumps(objectToWrite)) > 1000:
      raise Exception("Exceeded write limit")
    with open(self.stateFileLocation, 'w') as stateFile:
      pickle.dump(objectToWrite, stateFile)

  def readState(self):
    try:
      with open(self.stateFileLocation, 'r') as stateFile:
